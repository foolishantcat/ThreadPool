...
{
    int i = 0;
    for(Iterator it = idle_thread_container_.begin();
            it != idle_thread_container_.end();
            it++)
    {
        cout << i++ << endl;
        delete *it;
...
{
    idle_thread_container_.erase(std::find(idle_thread_container_.begin(), 
                idle_thread_container_.end(), m));
}
